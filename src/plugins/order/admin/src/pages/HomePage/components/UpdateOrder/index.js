...
const index = ({  OrderDelete, setOrderDelete, HandleDeleteOrder }) => {
  ...
  const [title, setTitle] = useState(OrderDelete.title);
  const [options, setOptions] = useState(OrderDelete.keywords);
  const keywordSchema = {
    keyword: '',
  };
  ...
  const HandleCreate = async () => {
    ...
    await HandleDeleteOrder(OrderDelete.id, data);
    setTitle('');
    setOptions([]);
  ...
  return (
    <>
      <ModalLayout onClose={() => setOrderDelete(false)} labelledBy="Tạo mục tiêu mới">
          <ModalHeader>
          ...
          <ModalFooter startActions={<Button onClick={() => setOrderDelete(false)} variant="tertiary">
                Hủy
