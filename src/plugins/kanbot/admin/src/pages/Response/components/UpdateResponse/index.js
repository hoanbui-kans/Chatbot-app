...
const index = ({ intents, entities, isLoading, ResponseUpdate, setResponseUpdate, HandleUpdateResponse }) => {
  console.log(ResponseUpdate);
  const [message, setMessage] = useState(ResponseUpdate.message);
  const [intent, setIntent] = useState({
    id: ResponseUpdate.intent.id.toString(),
    title: ResponseUpdate.intent.title,
    name: ResponseUpdate.intent.name
  });
  ...
  useEffect(() => {
    let NewOptions = [];
    if(Array.isArray(ResponseUpdate.entities) && ResponseUpdate.entities.length){
      ResponseUpdate.entities.map((val) => {
        NewOptions.push({
          keyword: {
            start: "",
            end: "",
  ...
  async function HandleCreate () {
    ...
    await HandleUpdateResponse(ResponseUpdate.id, data);
  ...
  return (
    <>
    <ModalLayout onClose={() => setResponseUpdate(false)} labelledBy="title">
                    <ModalHeader>
