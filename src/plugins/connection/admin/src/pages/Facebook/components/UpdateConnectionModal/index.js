...
const index = ({ ConnectionUpdate, setConnectionUpdate, HandleUpdateConnection }) => {

  const [title, setTitle] = useState(ConnectionUpdate.title);
  const [options, setOptions] = useState(ConnectionUpdate.keywords);
  const keywordSchema = {
    keyword: '',
  };
  ...
  const HandleCreate = async () => {
    ...
    await HandleUpdateConnection(ConnectionUpdate.id, data);
    setTitle('');
    setOptions([]);
  ...
  return (
    <>
      <ModalLayout onClose={() => setConnectionUpdate(false)} labelledBy="Tạo mục tiêu mới">
          <ModalHeader>
          ...
          <ModalFooter startActions={<Button onClick={() => setConnectionUpdate(false)} variant="tertiary">
                Hủy
